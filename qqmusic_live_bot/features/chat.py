...
from ..strategy.rules import LIMITS, TRIGGERS
...
class ChatFeature:
    def _is_triggered(self, event: Event) -> bool:
        ...
        if any(name.lower() in lowered for name in TRIGGERS["names"]):
            return True
        return any(keyword.lower() in lowered for keyword in TRIGGERS["keywords"])

    def select(self, events: list[Event], state: BotState, now_ts: float) -> ReplyAction | None:
        if now_ts - state.last_chat_time < LIMITS["chat_reply_interval"]:
            return None
        chats = [event for event in events if event.type == EventType.CHAT]
        for target in reversed(chats):
            if not self._is_triggered(target):
                continue
            if target.fingerprint in state.sent_fingerprints:
                continue
